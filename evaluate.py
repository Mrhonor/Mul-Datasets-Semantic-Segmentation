...
class MscEvalV0_CVCUDA(object):
    ...
    def __call__(self, net, dl, n_classes, dataset_id, device_id):
        # n_classes = 43
        ## evaluate
        hist = torch.zeros(n_classes, n_classes).cuda().detach()
        # hist = torch.zeros(118, 118).cuda().detach()
        ...
        while True:
            ...
            label = torch.as_tensor(
                label.cuda(), device="cuda:%d" % device_id
...
@torch.no_grad()
def eval_model_cvcuda(configer, net, device_id, cuda_ctx):
    org_aux = net.aux_mode
    net.aux_mode = 'eval'
...
def eval_model_mulbn(configer, net):
    ...
    single_scale = MscEvalV0_mulbn(configer, (0.5, ), False)
...
@torch.no_grad()
def eval_find_use_and_unuse_label_CVCUDA(configer, net, device_id, cuda_ctx):
        ## evaluate
    # hist = torch.zeros(n_classes, n_classes).cuda().detach()
    # datasets_remap = []
    org_aux = net.aux_mode
    ...
    bipart_graph = net.bipartite_graphs
    is_dist = dist.is_initialized()
    dls = getDataLoaderCVCUDA(configer, device_id, cuda_ctx, stage=2, mode='eval_link')
    ds = dls()
