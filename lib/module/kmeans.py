...
def initialize(X, num_clusters, rand_seed):
    """
    initialize cluster centers
    :param X: (torch.tensor) matrix
    :param num_clusters: (int) number of clusters
    :param rand_seed: (int) rand_seed for kmeans
    ...
    if rand_seed == None:
        ...
        np.random.seed(rand_seed) ; indices = np.random.choice(num_samples, num_clusters, replace=False)
...
def kmeans(
    ...
    if type(cluster_centers) == list:  # ToDo: make this less annoyingly weird
        initial_state = initialize(X, num_clusters, rand_seed=rand_seed)
    else:
        if tqdm_flag:
            print('resuming')
