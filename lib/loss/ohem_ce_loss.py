...
from lib.loss.loss_helper import RecallCrossEntropy, FocalLoss, AdjNLLPlusLoss
...
class MdsOhemNLLPlusLoss(nn.Module):

    def __init__(self, configer, thresh, ignore_lb=255):
        super(MdsOhemNLLPlusLoss, self).__init__()
        self.configer = configer
        self.n_datasets = self.configer.get('n_datasets')
        self.thresh = -torch.log(torch.tensor(thresh, requires_grad=False, dtype=torch.float)) #.cuda()
        self.ignore_lb = ignore_lb
        self.criteria = AdjNLLPlusLoss(ignore_lb=ignore_lb, reduction='none')
        # self.criteria = FocalLoss(gamma=2, ignore_index=ignore_lb, reduction='none')
        # self.criterias = [RecallCrossEntropy(n_classes=self.configer.get(f'dataset{i+1}', 'n_cats'), ignore_index=ignore_lb) for i in range(self.n_datasets)]

    def forward(self, logits, labels, bi_graphs, dataset_ids):
        if labels.is_cuda:
            self.thresh.cuda()
        ...
        for i in range(self.n_datasets):
            if not (dataset_ids == i).any():
                continue
            # loss = self.criterias[i](logits[cur_index], labels[dataset_ids==i]).view(-1)
            loss = self.criteria(logits[dataset_ids==i], bi_graphs[i], labels[dataset_ids==i]).view(-1).contiguous()
            loss_hard = loss[loss > self.thresh]
