...
class Learnable_Topology_BGAT(nn.Module):
    def __init__(self, configer):
        """Dense version of GAT."""
        super(Learnable_Topology_BGAT, self).__init__()
        ...
        # self.GCN_layer1 = GCN(self.nfeat_out, self.nfeat_out)
        self.GCN_layer1 = nn.ModuleList([GraphAttentionLayer(self.nfeat_out, self.nhid, dropout=self.dropout_rate, alpha=self.alpha, concat=True) for _ in range(self.nheads)])
        self.GCN_layer2 = nn.ModuleList([GraphAttentionLayer(self.nhid * self.nheads, self.nhid, dropout=self.dropout_rate, alpha=self.alpha, concat=True) for _ in range(self.nheads)])
        self.GCN_layer3 = GraphAttentionLayer(self.nhid * self.nheads, self.nfeat_out, dropout=self.dropout_rate, alpha=self.alpha, concat=True) 
    ...
    def forward(self, x, pretraining=False):
        ...
        feat_gcn1 = torch.cat([att(feat1_relu, adj_mI) for att in self.GCN_layer1], dim=1)
        feat_gcn2 = torch.cat([att(feat_gcn1+feat1_relu, adj_mI) for att in self.GCN_layer2], dim=1)
        feat_gcn3 = self.GCN_layer3(feat_gcn2+feat_gcn2, adj_mI)
        
        # feat4 = F.elu(feat_gcn3 + before_gcn3_x)
        # feat3_drop = F.dropout(feat3, self.dropout_rate, training=self.training)
        feat_out = self.linear1(feat_gcn3+feat_gcn3)
    ...
    def get_optimal_matching(self, x, init=False):
        ...
        feat_gcn1 = torch.cat([att(feat1_relu, adj_mI) for att in self.GCN_layer1], dim=1)
        feat_gcn2 = torch.cat([att(feat_gcn1+feat1_relu, adj_mI) for att in self.GCN_layer2], dim=1)
        feat_gcn3 = self.GCN_layer3(feat_gcn2+feat_gcn1, adj_mI)
