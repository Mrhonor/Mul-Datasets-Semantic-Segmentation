...
class AttentionRefinementModule(nn.Module):
    ...
    def forward(self, x):
        ...
        atten = feat.view(feat.shape[0], feat.shape[1], -1)
        atten = atten.mean(2)
        atten = atten.view(atten.shape[0], atten.shape[1], 1, 1)
        atten = self.conv_atten(atten)
        atten = self.bn_atten(atten)
        #  atten = self.sigmoid_atten(atten)
        atten = F.sigmoid(atten)
        out = feat * F.interpolate(atten, size=(feat.shape[2], feat.shape[3]))
        return out
...
class ContextPath(nn.Module):
    ...
    def forward(self, x):
        ...
        avg = feat32.view(feat32.shape[0], feat32.shape[1], -1)
        avg = avg.mean(2)
        avg = avg.view(avg.shape[0], avg.shape[1], 1, 1)
        avg = self.conv_avg(avg)

        feat32_arm = self.arm32(feat32)
        # feat32_sum = feat32_arm + avg
        feat32_sum = feat32_arm + F.interpolate(avg, size=(feat32_arm.shape[2], feat32_arm.shape[3]))
...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, fsp, fcp):
        ...
        atten = F.sigmoid(atten)
        feat_atten = feat * F.interpolate(atten, (feat.shape[2], feat.shape[3]))
