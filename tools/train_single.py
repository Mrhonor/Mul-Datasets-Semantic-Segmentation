...
def train():
    ...
    for i in range(0, cfg.max_iter):
        ...
        #     # loss_pre = [criteria_pre(logits[i][0], lb[i]) for i in range(0, n_dataset)]
        #     loss_aux = [[crit(lgt[i][0], lb[i]) for crit, lgt in zip(criteria_aux, logits_aux)] for i in range(0, n_dataset)]
        ...
        # loss_pre = [criteria_pre(logits[i], lb[i]) for i in range(0, n_dataset)]  # logits[i]仍是一个len为1的元组
        loss_pre = [criteria_pre(logits[i][0], lb[i]) for i in range(0, n_dataset)]
        loss_aux = [[crit(lgt[i][0], lb[i]) for crit, lgt in zip(criteria_aux, logits_aux)] for i in
                    range(0, n_dataset)]
        loss_all = [loss_pre[i] + sum(loss_aux[i]) for i in range(0, n_dataset)]
        # 不同数据集loss的权重
        w1 = 1
