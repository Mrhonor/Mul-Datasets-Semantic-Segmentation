...
from evaluate import eval_model_contrast, eval_model_aux, eval_model, eval_model_contrast_single, eval_model_mulbn, eval_model_dsg, eval_model_unlabel, eval_find_use_and_unuse_label, eval_model_cvcuda
...
def train():
    ...
    with cvcuda_stream, torch.cuda.stream(torch_stream):
        ...
        for i in range(1):
            ## model
            net = set_model(configer=configer, pth=f'res/celoss/snp_pretrain_model_50000.pth')
            if is_distributed():
                torch.cuda.empty_cache()
                heads, mious = eval_model_cvcuda(configer, net.module, device_id, cuda_ctx)
            else:
                torch.cuda.empty_cache()
