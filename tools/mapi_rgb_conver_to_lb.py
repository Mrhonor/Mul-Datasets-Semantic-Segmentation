...
#     # 读取JSON文件
#     with open(f'datasets/mapi/{mode}.txt', 'r') as f:
#         for i, line in enumerate(f):
#             imp, lbp = line.split(",")
#             lbp = lbp.replace('\n', '')

#             lb = np.array(Image.open(lbp).convert('RGB'))
#             # print(lb.shape)
#             imH, imW, _ = lb.shape
...
#                 # print(bgr)
#                 annotation_image[np.where(np.all(lb == color, axis=-1))] = id
#             Image.fromarray(annotation_image).save(lbp.replace('.png','_L.png'))    
       
    # # 输出每个category_id的RGB值
...
class MapiData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lbpth = self.lb_paths[idx].replace("_L.png", ".png")
...
if __name__ == "__main__":
    dataroot = "/cpfs01/projects-HDD/pujianxiangmuzu_HDD/pujian/mr/datasets/mapi"
    annpath = "datasets/mapi/validation_v1.txt"
