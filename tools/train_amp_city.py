...
def parse_args():
    ...
    parse.add_argument('--config', dest='config', type=str,
            default='/root/autodl-tmp/project/BiSeNet/configs/bisenetv2_city.py',)
...
def train():
    ...
    for it, (im, lb) in enumerate(dl):
        ...
        with amp.autocast(enabled=cfg.use_fp16):
            ...
            loss_pre = criteria_pre(logits[0], lb)
            loss_aux = [crit(lgt[0], lb) for crit, lgt in zip(criteria_aux, logits_aux)]
        ...
        if (it + 1) % 500 == 0:
            save_pth = osp.join(cfg.respth, 'model_{}.pth'.format(it+1))
