...
from lib.get_dataloader import get_data_loader, get_single_data_loader, get_data_loader_all_in_one
...
def train():
    ...
    dl = get_data_loader_all_in_one(configer, aux_mode='train', distributed=is_dist)
    ...
    for i in range(0, n_datasets):
        total_bs += configer.get('dataset'+str(i+1), 'ims_per_gpu')
    ...
    for i in range(starti, configer.get('lr','max_iter') + starti):
        ...
        try:
            im, lb, dataset_lbs = next(dl_iters)
            if not len(dataset_lbs) == total_bs:
                raise StopIteration
        except StopIteration:
            dl_iters = iter(dl)
            im, lb, dataset_lbs = next(dl_iters)
